...
// Parse KT
const breedRegex = /BreedStandard\([\s\S]+?combType\s*=\s*"[^"]+"\s*\)/g;
const breedsFromKT = [];
let match;
while ((match = breedRegex.exec(ktContent)) !== null) {
    ...
    if (id) {
        breedsFromKT.push({
            ...
            weightRoosterKg: parseFloat(wRooster || "2.5"), weightHenKg: parseFloat(wHen || "2.0"),
            official: official === 'true', recognizedBy, category, isTrueBantam: isTrueBantam === 'true'
        });
    }
}

console.log(`Parsed ${breedsFromKT.length} breeds from KT.`);

// Identify missing in TS
const existingRegex = /id:\s*"([^"]+)"/g;
const existingIds = new Set();
while ((match = existingRegex.exec(tsContent)) !== null) {
    existingIds.add(match[1]);
}

const missingBreeds = breedsFromKT.filter(b => !existingIds.has(b.id));
console.log(`Found ${missingBreeds.length} breeds missing from TS file.`);

if (missingBreeds.length === 0) {
    console.log("No breeds to add.");
    // Even if no breeds to add, we should ensure genetic patch applied to existing ones
    // But apply_genetics_patch.js already did that (partially, for the ones it found).
    // The ones it "didn't find" were the missing ones!
    process.exit(0);
}

// Generate TS entries
let newEntries = "\n    // --- BREEDS ADDED FROM BreedStandardRepository.kt (KT→TS sync) ---\n";

for (const breed of missingBreeds) {
    const geneticData = allGeneticData[breed.id] || {
        baselineGenotype: { "O_Locus": "o/o" }, // Minimal default
...
// Find insertion point - Last '];' before end of file
...
console.log(`Successfully appended ${missingBreeds.length} breeds.`);
