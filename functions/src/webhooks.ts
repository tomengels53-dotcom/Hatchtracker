...
interface SubscriptionUpdate {
    ...
    source: "stripe" | "paddle" | "lemon_squeezy";
}
 ...
 */
async function processSubscriptionEvent(update: SubscriptionUpdate) {
    ...
    await firestore.runTransaction(async (transaction) => {
        ...
        let newTier: SubscriptionTier = subscriptionTier;
        ...
        if (userDoc.exists) {
            ...
            if (
                currentData?.subscriptionTier === newTier &&
                currentData?.subscriptionSource === source &&
                currentData?.subscriptionStatus === status
            ) {
 ...
 */
function mapPaddlePlanToTier(planId: string): SubscriptionTier {
    if (planId.includes("expert") || planId === "12345") return "expert";
    if (planId.includes("pro") || planId === "67890") return "pro";
    return "free";
}
 ...
 */
function mapLemonPlanToTier(planName: string): SubscriptionTier {
    const formattedName = planName.toLowerCase();
    if (formattedName.includes("pro")) return "pro";
    if (formattedName.includes("expert")) return "expert";
    return "free";
}
...
export const stripeWebhook = functions.https.onRequest(async (req, res) => {
    ...
    try {
        if (
            ...
            const uid = subscription.metadata?.uid || subscription.metadata?.firebase_uid;
            ...
            let status: SubscriptionUpdate['status'] = "active";
            ...
            if (nickname.includes("pro")) tier = "pro";
            else if (nickname.includes("expert")) tier = "expert";
...
export const lemonSqueezyWebhook = functions.https.onRequest(async (req, res) => {
    try {
        const secret = process.env.LEMON_SQUEEZY_WEBHOOK_SECRET || functions.config().lemon_squeezy?.webhook_secret;
        if (!secret) {
